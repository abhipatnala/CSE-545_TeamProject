...
def getRoleBasedMenus(role):
    role_object = Roles.objects.filter(role_id = role)
    role_name =''
    if role_object!=NULL:
        role_name = role_object[0].role_name
    menuList = Menu_Mapping.objects.filter(role_id = role)
    context = {
        'role_name' : role_name,
        'menuList' : menuList,
    }
...
def appointments(request):
    role = 6
    context = getRoleBasedMenus(role)
